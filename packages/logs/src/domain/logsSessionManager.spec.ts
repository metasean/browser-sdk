...
import { LOGS_SESSION_KEY, LoggerTrackingType, startLogsSessionManager } from './logsSessionManager'
...
describe('logs session manager', () => {
  ...
  it('when tracked should store tracking type and session id', () => {
    ...
    startLogsSessionManager(configuration as Configuration, true)
  ...
  it('when no cookies available, isTracked is computed at each init and getId is undefined', () => {
    const sessionManager = startLogsSessionManager(configuration as Configuration, false)
  ...
  describe('findSession', () => {
    it('should return the current session', () => {
      setCookie(SESSION_COOKIE_NAME, 'id=abcdef&logs=1', DURATION)
      const logsSessionManager = startLogsSessionManager(configuration as Configuration, true)
